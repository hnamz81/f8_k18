...
import type { LoginResponse, LoginAccount, ApiResponse} from "../utils"
...
export const getData = <T>(endpoint: string) =>
handleRequest<T>(api.get(endpoint));

export const createData = <T>(endpoint: string, body: any) =>
handleRequest<T>(api.post(endpoint, body));

export const updateData = <T>(
    endpoint: string,
    id: number | string,
    body: any
) => handleRequest<T>(api.put(`${endpoint}/${id}`, body));

export const deleteData = <T>(
    endpoint: string,
    id: number | string
) => handleRequest<T>(api.delete(`${endpoint}/${id}`));